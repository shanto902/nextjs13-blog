...
const PostCard = async ({post, layout = "horizontal", reverse = false, locale}: PostProps) => {
  ...
  return (
    <Link className={`@container ${layout==='horizontal' ? "grid grid-cols-1 md:grid-cols-2 gap-10 items-center": "space-y-2"} `} href={`http://localhost:3000/${locale}/post/${post.slug}`}>
        {/* Post Image */}
        <div>
