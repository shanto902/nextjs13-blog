...
const UniversityList = ({ university , locale }: { university: University, locale:string }) => {
  return (
    <div className=" flex flex-col gap-4 my-4">
      <h3 className=" underline decoration-red-700 text-lg font-semibold">{university.tag_line}</h3>
     <div className=" grid grid-cols-3 ">
     {university.posts.map((post: StudentPost) => (
        <div key={post.id} className="">
          <Image className=" w-[380px] h-[280px] object-cover object-center"
            width={380}
        height={280}
        alt={post.title}
        src={`${process.env.NEXT_PUBLIC_ASSETS_URL}${post.image}?key=optimized`}
        placeholder={`data:image/svg+xml;base64,${toBase64(shimmer(380, 280))}`}
      >
