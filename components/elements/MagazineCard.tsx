...
const MagazineCard = ({magazine, collectMagazine, number, inputName, inputEmail, submitButton, locale}:{magazine:Magazine, collectMagazine:string , number:string, inputName:string, inputEmail:string, submitButton:string, locale:string}) => {
  ...
  return (
    ...
    <Image width={365} height={480} className="flex-1 max-w-[365px]" src={`${process.env.NEXT_PUBLIC_ASSETS_URL}${magazine.image}?key=optimized`} alt="image" />
    <div className="flex-1 flex flex-col gap-2">
      <h2 className=' text-5xl font-semibold'>{number} {getLocalizedPageNumber(magazine.number,locale)}</h2>
     ...
     <div>{formattedDescription}</div>
      ...
      <form className=" flex flex-col gap-5" action="">
        ...
        <input
          ...
          className="input input-bordered w-full max-w-xs"
        />
        <input className=" btn text-secondary hover:text-accent bg-accent w-fit" type="submit" value={submitButton} />
      </form>
    </div>
  </div>
  )
