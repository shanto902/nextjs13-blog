...
const SideLogo = ({ dictionary, locale }: { dictionary: any, locale:string }) => {
  const params = useParams();
  const pathname = usePathname();
  return (
   <>
   {
    (pathname === "/bn" || pathname === "/en") ? <div></div> :  <div className=" max-w-7xl relative">
    <div className=" absolute left-[-170px] top-16">
      ...
      <h2 className="text-right text-3xl font-semibold right-2 pt-4 absolute " >
        {(params.category === "heritage" &&
          ...
          (params.category === "arts" && dictionary.navigation.links.arts) ||
          (params.category === "news" && dictionary.navigation.links.news) ||
